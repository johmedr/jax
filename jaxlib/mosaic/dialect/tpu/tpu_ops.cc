...
LogicalResult MatmulOp::verify() {
  ...
  if (getTransposeLhs()) {
    emitOpError(
        "Illegal: lhs transpose not supported via this API - please use the "
  ...
  if (getDimensionNumbers().has_value()) {
    ...
    // Size 1 enforced by verifier.
    auto lhs_non_contracting_dim = lhs_non_contracting_dims[0];
    auto rhs_non_contracting_dim = rhs_non_contracting_dims[0];
    // Check that all non-contracting dimensions have been seen exactly once
    for (int i = 0; i < lhs_rank; ++i) {
      if (i == lhs_non_contracting_dim && !lhs_dims_seen_in_output[i]) {
    ...
    for (int i = 0; i < rhs_rank; ++i) {
      if (i == rhs_non_contracting_dim && !rhs_dims_seen_in_output[i]) {
