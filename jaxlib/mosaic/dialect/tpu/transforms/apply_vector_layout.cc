...
const llvm::StringMap<rule_type> &rules() {
  static const absl::NoDestructor<llvm::StringMap<rule_type>> rules = [] {
    auto rules = llvm::StringMap<rule_type>{
    ...
    return absl::NoDestructor<llvm::StringMap<rule_type>>(rules);
  }();
  return *rules;
}
