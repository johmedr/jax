...
class BufferedRef:
  ...
  def compute_gmem_slice(self, grid_indices) -> tuple[Any, ...]:
    ...
    return tuple(
        pl.ds(idx * size, size)
        for idx, size in zip(index_map(*grid_indices), self.spec.block_shape)
    )
...
def inc_grid(
  ...
  for idx, size in reversed(zip(indices, grid)):
    idx += carry
    carry = lax.div(idx, size)
    new_indices.append(lax.rem(idx, size))
...
def emit_pipeline(
  ...
  def scoped_pipeline(
    ...
    def loop_body(step, indices):
      ...
      jax.lax.cond(
          ...
          lambda: map(
              lambda bref: bref.copy_in(
                  fetch_slot,
                  inc_grid(indices, grid, max_concurrent_steps),
                  barrier_ref,
              ),
              in_brefs,
          ),
      ...
      return inc_grid(indices, grid, 1)
